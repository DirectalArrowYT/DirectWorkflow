    ...
    for pbone in ob.pose.bones:
        ...
        for constr in pbone.constraints:
            if constr.type == 'COPY_TRANSFORM':
                ...
                # Store the target and subtarget before removing the original constraint
                new_target = constr.target
                new_subtarget = constr.subtarget
                for cp_constr in (pbone.constraints.new('COPY_ROTATION'), 
                                 pbone.constraints.new('COPY_LOCATION'),
                                 pbone.constraints.new('COPY_SCALE')):
                    cp_constr.target = new_target
                    cp_constr.subtarget = new_subtarget
