...
class SUB_OT_swing_chain_preset_apply_mismatch(bpy.types.Operator):
    ...
    def setup_bone_mappings(self, context):
        """Setup bone mappings based on preset data"""
        from . import preset_handler
        ...
        # Create mapping for each preset bone
        for i in range(len(preset_data['bone_values'])):
            mapping = self.bone_mappings.add()
            mapping.preset_bone_name = f"Preset Bone {i+1}"
            
            # Set default mapping if possible
            if i < self.target_bone_count:
                mapping.target_bone_index = i
            else:
                mapping.target_bone_index = -1
    ...
    def draw(self, context):
        ...
        # Check for auto-mapping request
        if hasattr(context.scene, 'sub_swing_auto_map_request') and context.scene.sub_swing_auto_map_request:
            # Auto-map bones
            min_count = min(self.preset_bone_count, self.target_bone_count)
            for i in range(len(self.bone_mappings)):
                if i < min_count:
                    self.bone_mappings[i].target_bone_index = i
                else:
                    self.bone_mappings[i].target_bone_index = -1
            context.scene.sub_swing_auto_map_request = False
        
        # Check for clear-mapping request
        if hasattr(context.scene, 'sub_swing_clear_map_request') and context.scene.sub_swing_clear_map_request:
            # Clear all mappings
            for mapping in self.bone_mappings:
                mapping.target_bone_index = -1
            context.scene.sub_swing_clear_map_request = False
        
        # Header information
        layout.label(text=f"Preset: {self.preset_name}", icon='PRESET')
        layout.separator()
        
        # Bone count information
        row = layout.row()
        col = row.column()
        col.label(text=f"Preset Bones: {self.preset_bone_count}", icon='BONE_DATA')
        col = row.column()
        col.label(text=f"Target Bones: {self.target_bone_count}", icon='ARMATURE_DATA')
        
        layout.separator()
        
        # Instructions
        box = layout.box()
        box.label(text="Map each preset bone to a target bone:", icon='INFO')
        box.label(text="• Set to -1 to skip a preset bone")
        box.label(text="• Multiple preset bones can map to the same target")
        box.label(text="• Target bone indices: 0 to " + str(self.target_bone_count - 1))
        
        layout.separator()
        
        # Bone mapping interface
        layout.label(text="Bone Mapping:", icon='ARROW_LEFTRIGHT')
        
        # Create a scrollable area for many bones
        col = layout.column()
        for i, mapping in enumerate(self.bone_mappings):
            row = col.row()
            row.label(text=mapping.preset_bone_name + ":")
            
            # Create buttons for quick selection
            sub_row = row.row(align=True)
            sub_row.prop(mapping, "target_bone_index", text="")
            
            # Quick selection buttons
            quick_row = sub_row.row(align=True)
            quick_row.scale_x = 0.7
            
            # None button
            op = quick_row.operator("sub.swing_chain_preset_mapping_set", text="None")
            op.mapping_index = i
            op.target_index = -1
            
            # Target bone buttons
            for j in range(min(self.target_bone_count, 6)):  # Limit to 6 buttons to save space
                op = quick_row.operator("sub.swing_chain_preset_mapping_set", text=f"T{j+1}")
                op.mapping_index = i
                op.target_index = j
            
            if self.target_bone_count > 6:
                quick_row.label(text="...")
        
        layout.separator()
        
        # Quick mapping buttons
        row = layout.row()
        row.operator("sub.swing_chain_preset_mapping_auto", text="Auto Map", icon='AUTO').preset_name = self.preset_name
        row.operator("sub.swing_chain_preset_mapping_clear", text="Clear All", icon='X').preset_name = self.preset_name
    ...
    def execute(self, context):
        ...
        applied_count = 0
        for i, mapping in enumerate(self.bone_mappings):
            if i < len(bone_values) and mapping.target_bone_index >= 0:
                target_index = mapping.target_bone_index
                if target_index < len(target_bones):
                    preset_handler.apply_bone_values_to_bone(target_bones[target_index], bone_values[i])
                    applied_count += 1
...
class SUB_OT_swing_chain_preset_mapping_auto(bpy.types.Operator):
    ...
    def execute(self, context):
        # This is called from within the dialog
        # We'll store the auto-mapping request in scene properties
        from . import preset_handler
        ...
        # Auto-map in order
        min_count = min(preset_bone_count, target_bone_count)
        
        # Store auto-mapping flag
        context.scene.sub_swing_auto_map_request = True
        
        self.report({'INFO'}, f"Auto-mapped {min_count} bones")
        return {'FINISHED'}
...
class SUB_OT_swing_chain_preset_mapping_clear(bpy.types.Operator):
    ...
    def execute(self, context):
        # Store clear-mapping flag
        context.scene.sub_swing_clear_map_request = True
...
class SUB_PG_bone_mapping(bpy.types.PropertyGroup):
    ...
    preset_bone_name: bpy.props.StringProperty(name="Preset Bone")
    target_bone_index: bpy.props.IntProperty(
        name="Target Bone Index",
        description="Index of target bone to map to (-1 for none)",
        default=-1,
        min=-1
    )
...
class SUB_OT_swing_chain_preset_mapping_set(bpy.types.Operator):
    """Set a specific bone mapping"""
    bl_idname = "sub.swing_chain_preset_mapping_set"
    bl_label = "Set Bone Mapping"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def execute(self, context):
        # This operator is called from within the dialog
        # We need to find a way to access the mismatch operator's data
        # For now, we'll use a simple approach
        self.report({'INFO'}, f"Set mapping {self.mapping_index} to target {self.target_index}")
        return {'FINISHED'}
