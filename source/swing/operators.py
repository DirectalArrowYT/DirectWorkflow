...
class SUB_OP_swing_copy_collisions_from_rig(Operator):
    """Copy all collision objects (spheres, ovals, ellipsoids, capsules, planes)
    from a source armature onto the active armature.  Any collision whose
    referenced bone(s) do not exist on the target rig are silently skipped.
    Collisions whose name already exists on the target are also skipped."""

    bl_idname = 'sub.swing_copy_collisions_from_rig'
    bl_label = 'Copy Collisions from Rig'
    bl_description = (
        'Copy all swing collision objects (spheres, ovals, ellipsoids, capsules, planes) '
        'from another armature onto this one.  Collisions referencing bones that do not '
        'exist on this rig are skipped automatically.'
    )
    bl_options = {'REGISTER', 'UNDO'}

    source_armature: bpy.props.StringProperty(name='Source Armature', default='')
    ...
    def draw(self, context):
        self.layout.prop_search(self, 'source_armature', bpy.data, 'armatures', text='Source Rig', icon='ARMATURE_DATA')

    def execute(self, context):
        dst_arma_obj: Object = context.object
        dst_arma_data: bpy.types.Armature = dst_arma_obj.data
        dst_ssd: SUB_PG_sub_swing_data = dst_arma_data.sub_swing_data
        dst_bones = {b.name for b in dst_arma_data.bones}

        if not self.source_armature:
            self.report({'WARNING'}, 'No source armature selected.')
            return {'CANCELLED'}
        src_arma_data = bpy.data.armatures.get(self.source_armature)
        if src_arma_data is None:
            self.report({'WARNING'}, f'Armature {self.source_armature!r} not found.')
            return {'CANCELLED'}
        src_ssd: SUB_PG_sub_swing_data = src_arma_data.sub_swing_data

        # Find the scene armature object for source (needed for create_meshes helpers).
        src_arma_obj = next(
            (o for o in bpy.data.objects if o.type == 'ARMATURE' and o.data is src_arma_data),
            None,
        )

        master_col_props = get_swing_mesh_master_collection(context, dst_arma_obj).sub_swing_collection_props

        added = {'spheres': 0, 'ovals': 0, 'ellipsoids': 0, 'capsules': 0, 'planes': 0}
        skipped_bone = 0
        skipped_dupe = 0

        existing_names = {c.name for col in (dst_ssd.spheres, dst_ssd.ovals, dst_ssd.ellipsoids, dst_ssd.capsules, dst_ssd.planes) for c in col}

        # ── Spheres ──────────────────────────────────────────────────────────
        for src in src_ssd.spheres:
            if src.name in existing_names:
                skipped_dupe += 1
                continue
            if src.bone not in dst_bones:
                skipped_bone += 1
                continue
            new: SUB_PG_swing_sphere = dst_ssd.spheres.add()
            new.name   = src.name
            new.bone   = src.bone
            new.offset = src.offset
            new.radius = src.radius
            new.blender_object = create_meshes.make_sphere_object_2(
                new.name, new.radius, new.offset,
                dst_arma_data.bones.get(new.bone))
            idx = dst_ssd.spheres.find(new.name)
            parent_swing_child_to_parent_obj_armature_deform(dst_arma_obj, new.blender_object, 'SPHERE', idx)
            master_col_props.spheres_collection.objects.link(new.blender_object)
            add_hash_to_param_labels(new.name, new.name)
            add_hash_to_param_labels(new.bone.lower(), new.bone.lower())
            existing_names.add(new.name)
            added['spheres'] += 1

        # ── Ovals ────────────────────────────────────────────────────────────
        for src in src_ssd.ovals:
            if src.name in existing_names:
                skipped_dupe += 1
                continue
            if src.start_bone_name not in dst_bones or src.end_bone_name not in dst_bones:
                skipped_bone += 1
                continue
            new: SUB_PG_swing_oval = dst_ssd.ovals.add()
            new.name            = src.name
            new.start_bone_name = src.start_bone_name
            new.end_bone_name   = src.end_bone_name
            new.radius          = src.radius
            new.start_offset    = src.start_offset
            new.end_offset      = src.end_offset
            new.blender_object = create_meshes.make_capsule_object(
                name=new.name,
                start_radius=new.radius, end_radius=new.radius,
                start_offset=new.start_offset, end_offset=new.end_offset,
                start_bone=dst_arma_data.bones.get(new.start_bone_name),
                end_bone=dst_arma_data.bones.get(new.end_bone_name))
            idx = dst_ssd.ovals.find(new.name)
            parent_swing_child_to_parent_obj_armature_deform(dst_arma_obj, new.blender_object, 'OVAL', idx)
            master_col_props.ovals_collection.objects.link(new.blender_object)
            add_hash_to_param_labels(new.name, new.name)
            add_hash_to_param_labels(new.start_bone_name.lower(), new.start_bone_name.lower())
            add_hash_to_param_labels(new.end_bone_name.lower(), new.end_bone_name.lower())
            existing_names.add(new.name)
            added['ovals'] += 1

        # ── Ellipsoids ───────────────────────────────────────────────────────
        for src in src_ssd.ellipsoids:
            if src.name in existing_names:
                skipped_dupe += 1
                continue
            if src.bone_name not in dst_bones:
                skipped_bone += 1
                continue
            new: SUB_PG_swing_ellipsoid = dst_ssd.ellipsoids.add()
            new.name      = src.name
            new.bone_name = src.bone_name
            new.offset    = src.offset
            new.rotation  = src.rotation
            new.scale     = src.scale
            new.blender_object = create_meshes.make_ellipsoid_object(
                name=new.name, offset=new.offset, rotation=new.rotation,
                scale=new.scale, bone=dst_arma_data.bones.get(new.bone_name))
            idx = dst_ssd.ellipsoids.find(new.name)
            parent_swing_child_to_parent_obj_armature_deform(dst_arma_obj, new.blender_object, 'ELLIPSOID', idx)
            master_col_props.ellipsoids_collection.objects.link(new.blender_object)
            add_hash_to_param_labels(new.name, new.name)
            add_hash_to_param_labels(new.bone_name.lower(), new.bone_name.lower())
            existing_names.add(new.name)
            added['ellipsoids'] += 1

        # ── Capsules ─────────────────────────────────────────────────────────
        for src in src_ssd.capsules:
            if src.name in existing_names:
                skipped_dupe += 1
                continue
            if src.start_bone_name not in dst_bones or src.end_bone_name not in dst_bones:
                skipped_bone += 1
                continue
            new: SUB_PG_swing_capsule = dst_ssd.capsules.add()
            new.name            = src.name
            new.start_bone_name = src.start_bone_name
            new.end_bone_name   = src.end_bone_name
            new.start_offset    = src.start_offset
            new.end_offset      = src.end_offset
            new.start_radius    = src.start_radius
            new.end_radius      = src.end_radius
            new.blender_object = create_meshes.make_capsule_object(
                name=new.name,
                start_radius=new.start_radius, end_radius=new.end_radius,
                start_offset=new.start_offset, end_offset=new.end_offset,
                start_bone=dst_arma_data.bones.get(new.start_bone_name),
                end_bone=dst_arma_data.bones.get(new.end_bone_name))
            idx = dst_ssd.capsules.find(new.name)
            parent_swing_child_to_parent_obj_armature_deform(dst_arma_obj, new.blender_object, 'CAPSULE', idx)
            master_col_props.capsules_collection.objects.link(new.blender_object)
            add_hash_to_param_labels(new.name, new.name)
            add_hash_to_param_labels(new.start_bone_name.lower(), new.start_bone_name.lower())
            add_hash_to_param_labels(new.end_bone_name.lower(), new.end_bone_name.lower())
            existing_names.add(new.name)
        ...
        for src in src_ssd.planes:
            ...
            if src.bone_name not in dst_bones:
                skipped_bone += 1
                continue
            new: SUB_PG_swing_plane = dst_ssd.planes.add()
            new.name      = src.name
            new.bone_name = src.bone_name
            new.nx        = src.nx
            new.ny        = src.ny
            new.nz        = src.nz
            new.distance  = src.distance
            new.blender_object = create_meshes.make_plane_object(
                name=new.name, bone=dst_arma_data.bones.get(new.bone_name),
                nx=new.nx, ny=new.ny, nz=new.nz, d=new.distance)
            idx = dst_ssd.planes.find(new.name)
            parent_swing_child_to_parent_obj_armature_deform(dst_arma_obj, new.blender_object, 'PLANE', idx)
            master_col_props.planes_collection.objects.link(new.blender_object)
            add_hash_to_param_labels(new.name, new.name)
            add_hash_to_param_labels(new.bone_name.lower(), new.bone_name.lower())
            existing_names.add(new.name)
            added['planes'] += 1
        ...
        self.report(
            {'INFO'},
            f'Copied {total} collisions from {self.source_armature!r} '
            f'(spheres={added["spheres"]}, ovals={added["ovals"]}, '
            f'ellipsoids={added["ellipsoids"]}, capsules={added["capsules"]}, '
            f'planes={added["planes"]}). '
            f'Skipped: {skipped_bone} missing bones, {skipped_dupe} duplicates.',
        )
        return {'FINISHED'}
...
class SUB_OP_swing_copy_bone_collisions_from_rig(Operator):
    """Copy the per-swing-bone collision assignments from a source armature.

    'Copy Collisions from Rig' only copies the collision *shapes* - the spheres,
    capsules and so on. It does not touch the lists on each swing bone that say
    which of those shapes that bone actually collides with, so chains arrive on
    the target rig with an empty collision list. This copies those lists.

    Two things make it more than a straight copy:

    * A swing bone stores its collisions as (type, index) into the target rig's
      own collision list, NOT by name. The same shape almost always sits at a
      different index on each rig, so indices are re-resolved through the shape
      name. Copying the numbers verbatim would silently attach every bone to
      the wrong shapes.
    * Chain names rarely survive a transplant ('mantlefl' vs 'mantlefl1'), so
      chains are matched on their start bone, which does survive.
    """

    bl_idname = 'sub.swing_copy_bone_collisions_from_rig'
    bl_label = 'Copy Swing Bone Collisions from Rig'
    bl_description = (
        'Copy the collision assignments on each swing bone from another armature. '
        'Run "Copy Collisions from Rig" first so the shapes themselves exist here'
    )
    bl_options = {'REGISTER', 'UNDO'}

    source_armature: bpy.props.StringProperty(name='Source Armature', default='')
    match_by: bpy.props.EnumProperty(
        name='Match Chains By',
        items=(
            ('START_BONE', 'Start Bone', 'Match chains whose start bone name is the same. '
                                         'Survives chain renames like mantlefl -> mantlefl1'),
            ('CHAIN_NAME', 'Chain Name', 'Match chains by their name only'),
        ),
        default='START_BONE',
    )
    overwrite_existing: bpy.props.BoolProperty(
        name='Overwrite Chains That Already Have Collisions',
        description='Off: chains on this rig that already have collision assignments are left alone',
        default=False,
    )
    ...
    def draw(self, context):
        layout = self.layout
        layout.prop_search(self, 'source_armature', bpy.data, 'armatures',
                           text='Source Rig', icon='ARMATURE_DATA')
        layout.prop(self, 'match_by')
        layout.prop(self, 'overwrite_existing')

    def execute(self, context):
        dst_ssd: SUB_PG_sub_swing_data = context.object.data.sub_swing_data
        ...
        if src_arma_data is None:
            self.report({'WARNING'}, f'Armature {self.source_armature!r} not found.')
            return {'CANCELLED'}
        if src_arma_data is context.object.data:
            self.report({'WARNING'}, 'Source and target are the same armature.')
            return {'CANCELLED'}
        src_ssd: SUB_PG_sub_swing_data = src_arma_data.sub_swing_data

        # name -> index, per collision type, on the TARGET rig
        dst_index_by_name = {}
        for col_type, attr in _COLLISION_LISTS.items():
            dst_index_by_name[col_type] = {
                c.name.lower(): i for i, c in enumerate(getattr(dst_ssd, attr))
            }

        def find_dst_chain(src_chain):
            if self.match_by == 'START_BONE':
                key = src_chain.start_bone_name.lower()
                for chain in dst_ssd.swing_bone_chains:
                    if chain.start_bone_name.lower() == key:
                        return chain
                return None
            key = src_chain.name.lower()
            for chain in dst_ssd.swing_bone_chains:
                if chain.name.lower() == key:
                    return chain
            return None

        chains_done, chains_no_match, chains_skipped = 0, [], []
        refs_copied, bones_no_match = 0, 0
        missing_shapes = set()

        for src_chain in src_ssd.swing_bone_chains:
            if not any(len(b.collisions) for b in src_chain.swing_bones):
                continue                       # nothing to give
            dst_chain = find_dst_chain(src_chain)
            if dst_chain is None:
                chains_no_match.append(src_chain.name)
                continue
            if not self.overwrite_existing and any(len(b.collisions) for b in dst_chain.swing_bones):
                chains_skipped.append(dst_chain.name)
                continue

            dst_bone_by_name = {b.name.lower(): b for b in dst_chain.swing_bones}
            touched = False
            for src_bone in src_chain.swing_bones:
                dst_bone = dst_bone_by_name.get(src_bone.name.lower())
                if dst_bone is None:
                    bones_no_match += 1
                    continue
                dst_bone.collisions.clear()
                for src_ref in src_bone.collisions:
                    src_list = getattr(src_ssd, _COLLISION_LISTS[src_ref.collision_type], None)
                    if src_list is None or not (0 <= src_ref.collision_index < len(src_list)):
                        continue
                    shape_name = src_list[src_ref.collision_index].name
                    # Re-resolve through the name; the index differs per rig.
                    dst_index = dst_index_by_name[src_ref.collision_type].get(shape_name.lower())
                    if dst_index is None:
                        missing_shapes.add(f'{src_ref.collision_type.lower()}:{shape_name}')
                        continue
                    new_ref = dst_bone.collisions.add()
                    new_ref.collision_type = src_ref.collision_type
                    new_ref.collision_index = dst_index
                    refs_copied += 1
                    touched = True
            if touched:
                chains_done += 1

        msg = f'Copied {refs_copied} collision assignments across {chains_done} chain(s).'
        if chains_skipped:
            msg += (f' Left alone (already had collisions): {", ".join(chains_skipped)}.'
                    ' Enable Overwrite to replace them.')
        if chains_no_match:
            msg += f' No match on this rig for: {", ".join(chains_no_match)}.'
        if bones_no_match:
            msg += f' {bones_no_match} source bone(s) had no same-named bone here.'
        if missing_shapes:
            msg += (f' MISSING on this rig, run "Copy Collisions from Rig" first: '
                    f'{", ".join(sorted(missing_shapes))}.')
        self.report({'WARNING' if (missing_shapes or chains_no_match) else 'INFO'}, msg)
        print(f'[sub.swing_copy_bone_collisions_from_rig] {msg}')
        return {'FINISHED'}


class SUB_OP_swing_copy_chain_collisions(Operator):
    """Copy one chain's collision assignments onto the active chain.

    For chains that have no counterpart to inherit from - a back-centre skirt
    chain when the donor rig only has back-left and back-right, say - pick any
    chain with collisions that already work and hand them over.

    Bones are matched by position rather than by name, because the whole point
    is that the names differ (S_MantleBL1 -> S_MantleBC1). Chains of unequal
    length are stretched to fit, so a 6-bone chain can seed a 4-bone one.
    Collision indices are re-resolved through the shape name, so this is safe
    across rigs as well as within one.
    """

    bl_idname = 'sub.swing_copy_chain_collisions'
    bl_label = 'Copy Collisions From Another Chain'
    bl_description = (
        'Copy the collision assignments of another swing bone chain onto the active chain. '
        'Bones are matched by position, so chains with different bone names still work'
    )
    bl_options = {'REGISTER', 'UNDO'}

    source_armature: bpy.props.StringProperty(
        name='Source Rig',
        description='Leave blank to copy from a chain on this same rig',
        default='',
    )
    source_chain: bpy.props.StringProperty(name='Source Chain', default='')
    bone_match: bpy.props.EnumProperty(
        name='Match Bones By',
        items=(
            ('PROPORTIONAL', 'Position (stretch to fit)',
             'Spread the source chain evenly over the target chain. Identical to '
             'straight order when both chains are the same length'),
            ('ORDER', 'Position (1:1, truncate)',
             'First bone to first bone, and stop when the shorter chain runs out'),
            ('NAME', 'Bone Name',
             'Only copy where the bone names match exactly'),
        ),
    ...
    def _source_swing_data(self, context):
        arma = bpy.data.armatures.get(self.source_armature) if self.source_armature else None
        if arma is None:
            arma = context.object.data
        return arma, arma.sub_swing_data

    def draw(self, context):
        layout = self.layout
        dst_ssd = context.object.data.sub_swing_data
        active = dst_ssd.swing_bone_chains[dst_ssd.active_swing_bone_chain_index]
        layout.label(text=f'Onto active chain: {active.name}', icon='BONE_DATA')
        layout.prop_search(self, 'source_armature', bpy.data, 'armatures',
                           text='Source Rig', icon='ARMATURE_DATA')
        _, src_ssd = self._source_swing_data(context)
        layout.prop_search(self, 'source_chain', src_ssd, 'swing_bone_chains',
                           text='Source Chain', icon='LINKED')
        layout.prop(self, 'bone_match')

    def execute(self, context):
        dst_arma = context.object.data
        dst_ssd: SUB_PG_sub_swing_data = dst_arma.sub_swing_data
        dst_chain = dst_ssd.swing_bone_chains[dst_ssd.active_swing_bone_chain_index]

        src_arma, src_ssd = self._source_swing_data(context)
        if not self.source_chain:
            self.report({'WARNING'}, 'No source chain selected.')
            return {'CANCELLED'}
        src_chain = src_ssd.swing_bone_chains.get(self.source_chain)
        if src_chain is None:
            self.report({'WARNING'}, f'Chain {self.source_chain!r} not found on {src_arma.name!r}.')
            return {'CANCELLED'}
        if src_arma is dst_arma and src_chain.name == dst_chain.name:
            self.report({'WARNING'}, 'Source and target are the same chain.')
            return {'CANCELLED'}

        n_src = len(src_chain.swing_bones)
        n_dst = len(dst_chain.swing_bones)
        if n_src == 0 or n_dst == 0:
            self.report({'WARNING'}, 'One of the chains has no swing bones.')
            return {'CANCELLED'}
        ...
        src_by_name = {b.name.lower(): b for b in src_chain.swing_bones}

        def source_bone_for(j):
            if self.bone_match == 'NAME':
                return src_by_name.get(dst_chain.swing_bones[j].name.lower())
            if self.bone_match == 'ORDER':
                return src_chain.swing_bones[j] if j < n_src else None
            if n_dst == 1:
                return src_chain.swing_bones[0]
            i = int(round(j * (n_src - 1) / (n_dst - 1)))
            return src_chain.swing_bones[min(max(i, 0), n_src - 1)]

        refs_copied, bones_filled, bones_no_source = 0, 0, 0
        missing_shapes = set()

        for j, dst_bone in enumerate(dst_chain.swing_bones):
            src_bone = source_bone_for(j)
            if src_bone is None:
                bones_no_source += 1
                continue
            dst_bone.collisions.clear()
            for src_ref in src_bone.collisions:
                ...
                new_ref.collision_index = dst_index
                refs_copied += 1
            if len(dst_bone.collisions):
                bones_filled += 1

        msg = (f'Copied {refs_copied} collisions from {src_chain.name!r} '
               f'({n_src} bones) onto {dst_chain.name!r} ({n_dst} bones), '
               f'filling {bones_filled} bone(s).')
        if bones_no_source:
            msg += f' {bones_no_source} target bone(s) had no source bone.'
        if missing_shapes:
            msg += (' MISSING on this rig, run "Copy Collisions from Rig" first: '
                    f'{", ".join(sorted(missing_shapes))}.')
        self.report({'WARNING' if missing_shapes else 'INFO'}, msg)
        print(f'[sub.swing_copy_chain_collisions] {msg}')
        return {'FINISHED'}
