...
from ..anim.anim_data import SUB_PG_mat_track_property

from typing import TYPE_CHECKING
if TYPE_CHECKING:
    from ..anim.anim_data import SUB_PG_sub_anim_data, SUB_PG_mat_track, SUB_PG_mat_track_property
    ...
    def invoke(self, context, event):
        ...
        if context.space_data.type == 'VIEW_3D':
            ...
            if self.cv31_l is None and self.cv31_r is None:
                self.report({'WARNING'}, "Must load or create an animation first with `EyeL` or `EyeR` material tracks with the CustomVector31 property!")
                return {'CANCELLED'}            
